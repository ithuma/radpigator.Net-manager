...
memotemp.Clear;          // api/v2/folder/content?folder_id=7&token=arsd4h37ecfkdqd16uolf5na
...
if EditFolderNewName.text <> '' then
  ...
  memotemp.Clear;          // /api/v2/folder/create?name=test_folder&folder_id=191&token=0ji59
...
//for i := 0 to ( ListBoxFolder.Items.Count-1 ) do
//     ListBoxFolder.Items.Strings[i] := ListBoxFolder.Items.Strings[i] + ' (' + ini.ReadString( ListBoxFolder.Items.Strings[i], '0', '') + ')';
...
begin
tokennr := '';
memotemp.Clear;          // ?login=john@example.com&password=my_password
AUrl1 := 'https://rapidgator.net/api/v2/user/login?login='+lg+'&password='+ps;
AHTTPClient1 := TFPHTTPClient.Create(nil);
try
    AHTTPClient1.AllowRedirect := True;
      APostValues1 := TStringList.Create;
//      ApostValues1.Add('login='+lg);
//      ApostValues1.Add('password='+ps);
       try
        AResult1 := AHTTPClient1.SimpleGet(AUrl1);
        memoTemp.Lines.AddStrings(AResult1);
        except
...
begin
    ...
    //if tokennr='' then lb1.Items.Insert( 0 , ' login or password is wrong ') else lb1.Items.Insert( 0 , 'login correct');
    if admin = true then lb1.Items.Insert( 0 , 'token: ' + tokennr);
...
if ComboBoxFolder.Text ='' then showmessage('wybierz katalog') else
  //Labelchecklist.Caption := ini.ReadString( tylkonumerkatalogu( ComboBoxFolder.Text ), '0', '' ) ;
